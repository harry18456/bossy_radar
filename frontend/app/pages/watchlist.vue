...
// Get watched company codes (persisted in localStorage)
const watchedCodes = computed(() => watchlistStore.codes);
...
const sortedComparison = computed(() => {
  ...
  return companies.value
    .map((company) => {
      const data = allComparisonData.value?.find(
        (item) =>
      ...
      return {
        ...
        market_type: company.market_type, // Maintain market type if available
        industry: company.industry,
        violations_year_count: 0,
        violations_total_count: 0,
        env_violations_year_count: 0,
        env_violations_total_count: 0,
        non_manager_salary: undefined,
      } as YearlySummaryItem;
    })
...
<template>
  <div class="space-y-12">
    <!-- Entire page content wrapped in ClientOnly because it depends on localStorage -->
    <ClientOnly>
      <!-- Header -->
            ...
            >
              <thead class="bg-gray-50 dark:bg-slate-800">
                <tr>
                  <th
                    scope="col"
                    class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider sticky left-0 bg-gray-50 dark:bg-slate-800 z-10 whitespace-nowrap border-r border-gray-200 dark:border-slate-700"
                  >
                    公司名稱
                  </th>
                  <th
                    scope="col"
                    class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider whitespace-nowrap min-w-[120px]"
                  >
                    非主管平均薪資
                  </th>
                  <th
                  ...
                  <th
                    scope="col"
                    class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider whitespace-nowrap min-w-[80px]"
                  >
                    EPS
                  </th>
                  <th
                    scope="col"
                    class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider whitespace-nowrap min-w-[100px]"
                  >
                    勞動違規
                  </th>
                  <th
              ...
              >
                ...
                >
                  <td
                    class="px-6 py-4 whitespace-nowrap sticky left-0 bg-white dark:bg-slate-900 z-10 border-r border-gray-100 dark:border-slate-800"
                  >
                    <div class="flex items-center">
                      ...
                      <div
                        class="ml-2 text-xs text-gray-500 dark:text-slate-400"
                      >
                        {{ item.company_code }}
                      </div>
                      <NuxtLink
                  ...
                  >
                    {{
                      item.non_manager_salary?.avg_salary
                        ? formatCurrency(item.non_manager_salary.avg_salary)
                        : "-"
                    }}
                  </td>
                  <td
                    class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium text-yellow-600 dark:text-yellow-400"
                  >
                    {{
                      item.non_manager_salary?.median_salary
                        ? formatCurrency(item.non_manager_salary.median_salary)
                        : "-"
                    }}
                  </td>
                  <td
                  ...
                  <td
                    ...
                    :class="
                      (item.violations_year_count || 0) > 0
                  ...
                  >
                    ...
                    <span
                      v-if="item.violations_total_count"
                  ...
                  <td
                    class="px-6 py-4 whitespace-nowrap text-right text-sm"
                    :class="
                      (item.env_violations_year_count || 0) > 0
                  ...
                  >
                    ...
                    <span
                      v-if="item.env_violations_total_count"
                      class="text-xs font-normal text-gray-400 ml-1"
                    >
