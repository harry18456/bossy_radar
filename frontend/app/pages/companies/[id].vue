...
const {
  ...
  status,
  error,
} = await useAsyncData(`company-${id}`, () => api.getCompanyProfile(id));

const company = computed(() => profile.value?.company);

usePageMeta({
  title: company.value
    ? `${company.value.code} ${company.value.name}`
    : "公司詳情",
  description: company.value
    ? `${company.value.name} (${company.value.code}) 的薪資福利與違規紀錄。`
    : undefined,
});
...
const activeTab = ref<"overview" | "stats" | "violations" | "welfare">(
  "overview",
);
...
const isWatched = computed(() =>
  company.value ? watchlistStore.isWatching(company.value.code) : false,
);
...
<template>
  <div>
    ...
    <div v-else-if="profile && company" class="space-y-8">
      <!-- Header -->
      <div
        class="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-4 md:p-8 shadow-sm relative overflow-hidden"
      >
        ...
        >
          <div>
            <div class="flex items-center space-x-3 mb-2">
              ...
              <span
                class="text-sm font-medium px-2.5 py-0.5 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300"
              >
                {{ company.code }}
              </span>
            </div>
            <div
              class="flex flex-wrap items-center gap-4 text-sm text-gray-500 dark:text-slate-400"
            >
              <span
                class="flex items-center px-2 py-0.5 rounded bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-300 font-medium text-xs"
              >
      ...
      <!-- Tabs -->
      <div
        class="bg-gray-100 dark:bg-slate-800 p-1 rounded-xl flex md:bg-transparent md:dark:bg-transparent md:p-0 md:rounded-none md:border-b md:border-gray-200 md:dark:border-slate-800"
      >
        <nav
          class="flex w-full md:w-auto md:space-x-8 overflow-hidden"
          aria-label="Tabs"
        >
          <button
            v-for="tab in tabs"
      ...
      <div class="min-h-[400px]">
        ...
        <div
          ...
          class="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-4 md:p-6"
        >
          <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-6">
            公司基本資料
          </h3>
          <dl
            class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-4 md:gap-y-8"
          >
            <div>
          ...
          <div
            v-if="
              (profile.non_manager_salaries &&
                profile.non_manager_salaries.length > 0) ||
              (profile.salary_adjustments &&
        ...
        <div v-else-if="activeTab === 'violations'">
          ...
          <div v-if="violationType === 'labor'">
            <div v-if="profile.violations && profile.violations.length > 0">
              <!-- Desktop Table -->
              <div
                class="hidden md:block overflow-hidden bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl"
              >
                <table
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        <p
                          v-if="v.violation_content"
                          class="text-gray-500 dark:text-slate-400 text-xs whitespace-pre-wrap leading-relaxed"
                        >
                          {{ v.violation_content }}
                        </p>
                      </td>
                      ...
                      >
                        ...
                        <span
                          v-else
                          class="text-gray-400 dark:text-slate-500 font-normal"
                        >
                          -
                        </span>
                      </td>
              ...
              <div class="md:hidden space-y-4">
                <div
                  v-for="v in profile.violations"
                  :key="v.id"
                  class="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-4 shadow-sm"
                >
                  ...
                  <p
                    v-if="v.violation_content"
                    class="text-xs text-gray-600 dark:text-slate-400 bg-gray-50 dark:bg-slate-800/50 p-2 rounded-lg leading-relaxed mt-2"
                  >
                    {{ v.violation_content }}
                  </p>
                </div>
          ...
          <div v-if="violationType === 'env'">
            <div
              v-if="
                profile.environmental_violations &&
                profile.environmental_violations.length > 0
              "
            >
              <!-- Desktop Table -->
              <div
              ...
              >
                ...
                >
                  ...
                  >
                    <tr
                      v-for="ev in profile.environmental_violations"
                      :key="ev.id"
                      class="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors"
                    >
                      <td
                      ...
                      >
                        <div
                          class="font-medium mb-1 text-green-700 dark:text-green-400"
                        >
                          {{ ev.law_article }}
                        </div>
                        <p
                          v-if="ev.violation_reason"
                          class="text-gray-500 dark:text-slate-400 text-xs whitespace-pre-wrap leading-relaxed"
                        >
                          {{ ev.violation_reason }}
                        </p>
              ...
              <div class="md:hidden space-y-4">
                ...
                >
                  ...
                  <div class="mb-2">
                    ...
                    >
                      {{ ev.authority }} ({{ ev.disposition_no }})
                    </div>
                    <div
                      class="text-sm font-bold text-gray-900 dark:text-white"
                    >
        ...
        <div v-else-if="activeTab === 'welfare'">
          ...
          >
            <!-- Data Source Note -->
            <div
              class="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-lg flex items-start gap-3"
            >
              <Icon
                name="lucide:info"
              ...
              <div class="text-sm text-blue-800 dark:text-blue-300">
                <p class="mb-2">
                  此頁面資料彙整自公開資訊觀測站「員工福利政策及權益維護措施揭露-彙總資料查詢」。部分公司可能未完整填寫特定欄位導致資訊顯示不全。
                </p>
                <a
            ...
            <div
              v-if="
                profile.welfare_policies && profile.welfare_policies.length > 0
              "
            >
              <div
                v-for="policy in profile.welfare_policies"
                :key="policy.id"
                class="mb-8 last:mb-0 border-b border-gray-100 dark:border-slate-800 last:border-0 pb-8 last:pb-0"
              >
                <div class="flex items-center gap-3 mb-6">
                  <span
                    class="px-3 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-full text-sm font-bold"
                  >
                    {{ policy.year }}年度
                  </span>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div class="space-y-4">
                    <h5
                      class="flex items-center text-sm font-bold text-gray-900 dark:text-white pb-2 border-b border-gray-100 dark:border-slate-800"
                    >
                      <Icon
                    ...
                    <div class="grid grid-cols-2 gap-4">
                      <div
                        class="bg-gray-50 dark:bg-slate-800/50 p-3 rounded-lg"
                      >
                        <span
                          class="text-xs text-gray-500 dark:text-slate-400 block mb-1"
                          >預計調薪</span
                        >
                        <div class="font-medium text-gray-900 dark:text-white">
                          {{ policy.planned_salary_increase || "-" }}
                        </div>
                        <p
                          v-if="
                            policy.planned_salary_increase_note &&
                            policy.planned_salary_increase_note !==
                              policy.planned_salary_increase
                          "
                      ...
                      >
                        ...
                        >
                        <div class="font-medium text-gray-900 dark:text-white">
                          {{ policy.actual_salary_increase || "-" }}
                        </div>
                        <p
                          v-if="policy.actual_salary_increase_note"
                          class="text-xs text-gray-400 mt-1"
                        >
                          {{ policy.actual_salary_increase_note }}
                        </p>
                      </div>
                      <div
                        class="bg-gray-50 dark:bg-slate-800/50 p-3 rounded-lg"
                      >
                        ...
                        >
                        <div class="font-medium text-gray-900 dark:text-white">
                          {{ policy.non_manager_salary_increase || "-" }}
                        </div>
                        <p
                          v-if="policy.non_manager_salary_increase_note"
                          class="text-xs text-gray-400 mt-1"
                        >
                          {{ policy.non_manager_salary_increase_note }}
                        </p>
                      </div>
                      <div
                        class="bg-gray-50 dark:bg-slate-800/50 p-3 rounded-lg"
                      >
                        ...
                        >
                        <div class="font-medium text-gray-900 dark:text-white">
                          {{ policy.manager_salary_increase || "-" }}
                        </div>
                        <p
                          v-if="policy.manager_salary_increase_note"
                          class="text-xs text-gray-400 mt-1"
                        >
                          {{ policy.manager_salary_increase_note }}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="space-y-4">
                    ...
                    <div class="space-y-3">
                      <div
                        class="flex items-center justify-between p-3 bg-gray-50 dark:bg-slate-800/50 rounded-lg"
                      >
                      ...
                      >
                        ...
                        <span
                          class="font-medium text-gray-900 dark:text-white"
                          >{{
                            policy.entry_salary_highschool
                              ? `$${policy.entry_salary_highschool}`
                              : "-"
                          }}</span
                        >
                      ...
                      <div
                        v-if="policy.entry_salary_note"
                        class="p-3 bg-blue-50 dark:bg-blue-900/10 text-xs text-blue-700 dark:text-blue-300 rounded-lg"
                      >
                        <span class="font-bold mr-1">備註:</span
                        >{{ policy.entry_salary_note }}
                      </div>
                    </div>
                  </div>
                </div>
