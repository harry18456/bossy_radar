...
const { data: company, status, error } = await useAsyncData(
  `company-${id}`,
...
<template>
  <div>
    ...
    <div v-else-if="company" class="space-y-8">
      <!-- Header -->
      <div class="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-8 shadow-sm relative overflow-hidden">
        ...
        <div class="relative z-10 flex flex-col md:flex-row md:items-start justify-between gap-6">
          ...
          <div class="flex items-center space-x-3">
            ...
            >
              <Icon 
                :name="isWatched ? 'lucide:heart' : 'lucide:heart'" 
                class="w-5 h-5 mr-2" 
                :class="isWatched ? 'fill-current' : ''"
              />
              {{ isWatched ? '取消追蹤' : '加入追蹤' }}
            </button>
            
            <a 
              :href="`https://tw.stock.yahoo.com/quote/${company.code}`"
              target="_blank"
              class="flex items-center px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white transition-colors shadow-sm"
            >
      ...
      <div class="min-h-[400px]">
        <!-- Overview Tab -->
        <div v-if="activeTab === 'overview'" class="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-6">
        ...
        <div v-else-if="activeTab === 'stats'">
           <div v-if="company.yearly_stats && company.yearly_stats.length > 0">
              <CompanyYearlyStats :stats="company.yearly_stats" />
           </div>
        ...
        <div v-else-if="activeTab === 'violations'">
          <div v-if="company.violations && company.violations.length > 0" class="overflow-hidden bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl">
             <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-800">
               ...
               <tbody class="bg-white dark:bg-slate-900 divide-y divide-gray-200 dark:divide-slate-800">
                 <tr v-for="v in company.violations" :key="v.id" class="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                   ...
                   <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-slate-400">
                    ...
                    <td class="px-6 py-4 text-sm text-gray-900 dark:text-white">
                     {{ v.law_violated }}
                     <p v-if="v.violation_content" class="text-gray-500 dark:text-slate-400 text-xs mt-1 truncate max-w-xs">{{ v.violation_content }}</p>
                   </td>
        ...
        <div v-else-if="activeTab === 'welfare'">
          <div class="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-6">
             <div v-if="company.welfare_policies && company.welfare_policies.length > 0">
               <div v-for="policy in company.welfare_policies" :key="policy.id" class="prose dark:prose-invert max-w-none">
                 <h4 class="font-bold mb-2">{{ policy.year }}年 福利政策</h4>
                 <div class="whitespace-pre-line text-sm text-gray-700 dark:text-slate-300">{{ policy.welfare_content }}</div>
               </div>
             </div>
