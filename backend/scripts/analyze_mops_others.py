...
CONFIGS = {
    ...
    "t100sb13": {
        ...
        "firstin": "ture",
        "extra": {"off": "1"},
    },  # Note: 'ture' typo in original code
    "t222sb01": {
        "year_param": "RYEAR",
        "step": "1",
        "firstin": "1",
...
def fetch_and_analyze(source, year, market):
    ...
    if cache_path.exists():
        ...
        try:
            resp = httpx.post(f"{MOPS_BASE_URL}/{endpoint}", data=payload, timeout=30)
            html = resp.text
    ...
    if not col_counts:
        ...
        if major_counts:
            logger.info(
                f"{source} Year {year} {market}: Major column counts: {major_counts}"
            )
        else:
