...
def find_corrupted_records(session: Session):
    ...
    for c in companies:
        ...
        if issues:
            corrupted.append({"code": c.code, "name": c.name, "issues": issues})
...
def fix_specific_records(session: Session):
    ...
    for code, fixes in known_issues.items():
        ...
        if company.market_type and re.match(
            r"^(https?://|www\.)", company.market_type, re.IGNORECASE
        ):
            old_value = company.market_type
            company.market_type = fixes.get("expected_market_type", "Public")
            fixes_applied.append(
                f"{code}: Fixed market_type from '{old_value}' to '{company.market_type}'"
            )

        # If website was in wrong field, ensure it's in the right place
        if "website" in fixes and (
            not company.website or company.website != fixes["website"]
        ):
