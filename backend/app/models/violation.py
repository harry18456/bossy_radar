...
class Violation(SQLModel, table=True):
    ...
    # Dates
    penalty_date: Optional[date] = Field(
        default=None, index=True, description="處分日期"
    )
    announcement_date: Optional[date] = Field(
        default=None, index=True, description="公告日期"
    )

    # Violation Details
    disposition_no: Optional[str] = Field(
        default=None, index=True, description="處分字號"
    )
