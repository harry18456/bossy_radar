            ...
            for row in data:
                try:
                    ...
                    violation = EnvironmentalViolation(
                        ...
                        disposition_no=self._clean_str(
                            row.get("document_no")
                        ),  # 裁處書字號
                        # 事業資料
                        company_name=self._clean_str(row.get("fac_name"))
                        or "",  # 事業名稱
                        company_address=self._clean_str(
                            row.get("fac_address")
                        ),  # 公司（工廠）地址
                        violation_address=self._clean_str(
                            row.get("transgress_address")
                        ),  # 違反地址
                        # 違規資訊
                        violation_type=self._clean_str(
                            row.get("transgress_type")
                        ),  # 污染類別
                        violation_date=self._parse_date(
                            row.get("transgress_date")
                        ),  # 違反時間
                        violation_reason=self._clean_str(
                            row.get("openinfor")
                        ),  # 違反事實
                        law_article=self._clean_str(
                            row.get("transgress_law")
                        ),  # 違反法令
                        # 裁處資訊
                        authority=self._clean_str(row.get("county_name")),  # 裁處機關
                        penalty_date=self._parse_date(
                            row.get("penalty_date")
                        ),  # 裁處時間
                        fine_amount=self._parse_amount(
                            row.get("penalty_money")
                        ),  # 裁處金額
                        penalty_reason=self._clean_str(
                            row.get("gist_define")
                        ),  # 裁處理由及法令
                        # 後續處理
                        limit_date=self._parse_date(
                            row.get("improve_deadline")
                        ),  # 限改日期
                        is_improved=self._parse_bool(
                            row.get("is_improve")
                        ),  # 改善完妥與否
                        is_appeal=self._parse_bool(
                            row.get("ispetition")
                        ),  # 是否訴願訴訟
                        appeal_result=self._clean_str(
                            row.get("petition_results")
                        ),  # 訴願訴訟結果
                        is_paid=self._parse_bool(
                            row.get("paymentstate")
                        ),  # 罰鍰是否繳清
                        # 其他
                        illegal_profit=self._parse_amount(
                            row.get("illegal_money")
                        ),  # 不法利得
                        other_penalty=self._clean_str(
                            row.get("penaltykind")
                        ),  # 其他處罰方式
