...
def read_environmental_violations(
    ...
    violation_type: list[str] | None = Query(None, description="污染類別過濾"),
    authority: list[str] | None = Query(None, description="裁處機關過濾"),
    ...
    if violation_type:
        query = query.where(
            col(EnvironmentalViolation.violation_type).in_(violation_type)
        )
    ...
    if year:
        query = query.where(
            extract("year", EnvironmentalViolation.penalty_date).in_(year)
        )
        ...
        for sort_field in sort:
            ...
            if hasattr(EnvironmentalViolation, field_name):
                query = query.order_by(
                    direction(getattr(EnvironmentalViolation, field_name))
                )
    else:
        # Default sort
        query = query.order_by(
            desc(EnvironmentalViolation.penalty_date), desc(EnvironmentalViolation.id)
        )
