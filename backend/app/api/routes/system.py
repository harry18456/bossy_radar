...
def get_sync_status(session: SessionDep):
    ...
    if salary_data:
        mops_status["Salary"] = SyncStatusItem(
            last_updated=salary_data[0], count=salary_data[1]
        )
    ...
    if benefit_data:
        mops_status["Benefit"] = SyncStatusItem(
            last_updated=benefit_data[0], count=benefit_data[1]
        )
    ...
    if welfare_data:
        mops_status["Welfare"] = SyncStatusItem(
            last_updated=welfare_data[0], count=welfare_data[1]
        )
    ...
    if adjustment_data:
        mops_status["Adjustment"] = SyncStatusItem(
            last_updated=adjustment_data[0], count=adjustment_data[1]
        )
