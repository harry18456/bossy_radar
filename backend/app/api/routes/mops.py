...
@router.get(
    "/employee-benefits", response_model=PaginatedResponse[EmployeeBenefitResponse]
)
def read_employee_benefits(
    ...
    return PaginatedResponse(
        items=results, total=total, page=page, size=size, total_pages=total_pages
    )


# ========== Non-Manager Salaries (t100sb15) ==========
@router.get(
    "/non-manager-salaries", response_model=PaginatedResponse[NonManagerSalaryResponse]
)
...
@router.get(
    "/welfare-policies", response_model=PaginatedResponse[WelfarePolicyResponse]
)
...
@router.get(
    "/salary-adjustments", response_model=PaginatedResponse[SalaryAdjustmentResponse]
)
