...
def get_leaderboards(session: SessionDep):
    ...
    for year_roc, data in salary_data.items():
        salary[year_roc] = SalaryLeaderboard(
            ...
            bottom_by_median=[
                _to_salary_item(s, company_map) for s in data["bottom_median"]
            ],
        )
    ...
    for year_roc, industries in salary_by_industry_data.items():
        ...
        for industry, data in industries.items():
            salary_by_industry[year_roc][industry] = IndustrySalaryLeaderboard(
                top_by_median=[
                    _to_industry_salary_item(s, company_map) for s in data["top"]
                ],
                bottom_by_median=[
                    _to_industry_salary_item(s, company_map) for s in data["bottom"]
                ],
                top_by_eps=[
                    _to_industry_salary_item(s, company_map) for s in data["top_eps"]
                ],
                bottom_by_eps=[
                    _to_industry_salary_item(s, company_map) for s in data["bottom_eps"]
                ],
            )
